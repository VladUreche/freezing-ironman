...
object MacroVector3D {
  ...
  def benchmark_impl(c: Context)(tpe: c.Expr[BenchType]): c.Expr[Unit] = {
    ...
    def parameter: TypeDef = tpe.tree.symbol.name.toString match {
    ...
    val benchmarks: List[Tree] =
      for (T <- List(tq"Int", tq"Double", tq"Long")) yield {
        ...
        q"""
            object $name {
              class Vector3D[$parameter](val x: T, val y: T, val z: T)(implicit nm: MyNumeric[T]) {
            ...
            val vector3D: Vector3D[$T] = new Vector3D[$T](${elementsThis(0)}, ${elementsThis(1)}, ${elementsThis(2)})
            val vector3DGen: org.scalameter.Gen[Vector3D[$T]] = Gen.single("Vector3D")(vector3D)
