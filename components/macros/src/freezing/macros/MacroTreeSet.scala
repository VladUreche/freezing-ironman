  ...
  def benchmark_impl(c: Context)(tpe: c.Expr[BenchType]): c.Expr[Unit] = {
    ...
    val target: Tree = q"""
      object $name {
        ...
        implicit object MyOrderingString extends MyOrdering[String] {
          def gt(x: String, y: String): Boolean = x > y
          def lt(x: String, y: String): Boolean = x < y
          def eq(x: String, y: String): Boolean = x == y
        }

        implicit object MyOrderingLong extends MyOrdering[Long] {
          def gt(x: Long, y: Long): Boolean = x > y
